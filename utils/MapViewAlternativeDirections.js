...
class MapViewAlternativeDirections extends Component {
	...
	fetchAndRenderRoute = (props) => {
		...
		if (!apikey) {
			console.warn(`MapViewAlternativeDirections Error: Missing API Key`); // eslint-disable-line no-console
			return;
		}
		...
		Promise.all(routes.map((route, index) => {
			...
			return (
				this.fetchRoute(directionsServiceBaseUrl, origin, waypoints, destination, apikey, mode, language, region, precision, timePrecisionString, channel, onReady)
					.then(result => {
						return result;
					})
					.catch(errorMessage => {
			...
			// const result = results[0].reduce((acc, { distance, duration, coordinates, fare, legs, waypointOrder }) => {
			// 	acc.coordinates = [
			// 		...acc.coordinates,
			// 		...coordinates,
			// 	];
			...
			.catch(errorMessage => {
				this.resetState();
				console.warn(`MapViewAlternativeDirections Error: ${errorMessage}`); // eslint-disable-line no-console
				onError && onError(errorMessage);
			});
	}

	fetchRoute(directionsServiceBaseUrl, origin, waypoints, destination, apikey, mode, language, region, precision, timePrecision, channel, onReady) {
		...
		if (typeof (directionsServiceBaseUrl) === 'string') {
			...
			url += `&alternatives=true`; // Already added to fetch alternative routes
		}
	...
	render() {
		// console.log("ESTADO NO RENDER")
		// console.log(this.state);
		const { routes } = this.state;

		if (!routes || routes.length === 0) {
		  return null; // Se não houver rotas, não renderiza nada
		}
		...
		// Renderizando uma Polyline para cada rota
		return (
		  <>
			{/* {routes.map((route, index) => (
			  <Polyline
				key={index} // A chave é importante para a renderização correta em listas
				coordinates={route.coordinates}
				{...props} // Propaga as outras propriedades para cada Polyline
			  />
			))} */}
		  </>
		);
...
MapViewAlternativeDirections.propTypes = {
...
export default MapViewAlternativeDirections;